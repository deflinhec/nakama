...
export class EmailVerificationLinkComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (!environment.nt) {
      this.segment.page('/email-verification');
    }
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (this.token) {
        this.appService.verifyEmailAddress('', this.token)
          .subscribe(d => {
            this.updated = 'Email verification success.';
            this.error = '';
          }, err => {
            this.updated = '';
  ...
  onSubmit(): void {
    ...
    this.appService.sendEmailVerificationLink('', body)
      .subscribe(d => {
